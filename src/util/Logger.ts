...
export default class Logger {
    ...
    private raw(...args: string[]) {
        ...
        this.output(`[${this.getDate()}] <${this.name}>`, ...args);
    }
    ...
    public error(e: Error | string, stack: boolean = true) {
        ...
        this.output(`[${this.getDate().white.bold}] ${`ERROR<${this.name}>`.bgRed.bold}`, e.message);
    ...
    public warn(...args: string[]) {
        ...
        this.output(`[${this.getDate().white.bold}] ${`WARN<${this.name}>`.bgYellow.bold}`, ...args);
    }

    public debug(...args: any) {
        if (Logger.VERBOSE_LEVEL < VerboseLevel.ALL) return;
        console.log(`[${this.getDate().white.bold}] ${`DEBUG<${this.name}>`.bgBlue.bold}`, ...args);
        this.output(`[${this.getDate().white.bold}] ${`DEBUG<${this.name}>`.bgBlue.bold}`, ...args);
    ...
    public verbL(...args: any) {
        ...
        this.output(`[${this.getDate().white.bold}] ${`VERBL<${this.name}>`.bgCyan.bold}`, ...args);
        this.trail(new Error().stack!.split('\n').slice(2).join('\n'));
    }
    ...
    public verbH(...args: any) {
        ...
        this.output(`[${this.getDate().white.bold}] ${`VERBH<${this.name}>`.bgCyan.bold}`, ...args);
        this.trail(new Error().stack!.split('\n').slice(2).join('\n'));
    }

    private output(...args: any[]) {

        let logFile = './logs/' + new Date().toISOString().slice(0, 10) + '.log'
        let output = ''
